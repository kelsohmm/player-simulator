...
class VmHost:
    _BITMAP_FORMAT = BitmapFormat.bgr
    _ONLY_SCREEN_ID = 0
    _SNAP_RESTORE_TIMEOUT = 30 * 1000
    _VM_STARTUP_TIMEOUT = 60 * 1000
    _GUEST_SESSION_NAME = '__GUEST_SESSION__'

    def __init__(self, vm_name, snap_name, username, password, window_id):
        ...
        guest_session = self.session.console.guest.create_session(user=username, password=password)
    ...
    def take_screen_shot(self):
        ...
        h, w, _, _, _, _ = display.get_screen_resolution(self._ONLY_SCREEN_ID)


        return display.take_screen_shot_to_array(self._ONLY_SCREEN_ID, h, w, self._BITMAP_FORMAT)
...
VM_NAME = 'mariosnap'
SNAP_NAME = 'mariosnap3'

host = VmHost(VM_NAME, SNAP_NAME, 'stefancz', '123', "0x1200aff")

sleep(2)

host.keys_down(['RIGHT'])

sleep(5)
host.keys_up(['RIGHT'])

png = host.take_screen_shot()
